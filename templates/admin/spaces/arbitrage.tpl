<h2 class="page-title">
  {if $data.id}Редактирование источника `{$data.name}`{else}Добавление источника - Арбитраж{/if}
</h2>
...
<div class="page-bar">
  <ul class="page-breadcrumb">
    ...
    <li>
      <a href="#">{if $data.id}Редактирование источника `{$data.name}`{else}Добавление источника - Арбитраж{/if}</a>
    </li>
  </ul>
</div>
  ...
  <div class="portlet-body">
    <form action="/admin/spaces" method="post" id="space-form">
      ...
      <div class="form-horizontal">
        <input type="hidden" name="space[type]" value="arbitrage">
