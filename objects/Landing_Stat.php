...
class Landing_Stat extends Stat {
    ...
    protected function fetchItems(){
        if ($this->user_id == 0) {
            $query = "SELECT t1.c_id as id, t1.name
                      FROM content as t1
                      WHERE t1.type = 'landing'";
        } else {
            $query = "SELECT t1.c_id as id, t1.name
                      FROM content as t1 INNER JOIN pfinder_keys as t2 ON t1.c_id = t2.landing_id
    ...
    protected function getStatQuery(){
        if (!empty($this->filters["subid"])) {
            $query = "SELECT SUM(t1.`unique`) as `unique`, SUM(t1.`all`) as `all`, t2.landing_id as id
        ...
        // офферы
        if (!empty($this->ids)) {
            $query .=  " AND t2.landing_id  IN ( " . implode(",", $this->ids) . ")";
        }

        return $this->addStatFilters($query) . " GROUP BY t2.landing_id";
    }
    ...
    protected function getOrdersQueryParameters(){
        ...
        if (!empty($this->ids)) {
            $parameters[] = "t2.landing_id IN (" . implode(",", $this->ids) . ")";
        }
    ...
    protected function getOrdersQuery($parameters){
        ...
        $query = "SELECT t1.webmaster_commission as amount, t1.id as c, t1.country_code, t2.landing_id as id, t1.status, t1.target_close, t1.status2, t1.target FROM orders as t1 INNER JOIN pfinder_keys as t2 ON t1.pfinder_id = t2.pfinder_id";
